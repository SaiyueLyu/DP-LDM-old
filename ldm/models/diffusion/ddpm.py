...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
                 ...
                 scale_factor=1.0,
                 scale_by_std=False,
                 retrain_attention=False,  #transdiff
                 train_attention_only=False,  #transdiff
                 attention_flag='spatial',  #transdiff
                 condition_mask=0,   #transdiff
                 diffusion_mask=0,  #transdiff
                 train_condition_only=False,  #transdiff
                 ablation_blocks=-1,   #transdiff
                 dp_config=None,
                 *args, **kwargs):
    ...
    def configure_optimizers(self): ##### check transdiff
        ...
        if self.dp_config and self.dp_config.enabled:
            ...
            if hasattr(data_loader, "sample_rate"):
                self.sample_rate = data_loader.sample_rate
            else:
                self.sample_rate = data_loader.batch_size / len(data_loader.dataset)
